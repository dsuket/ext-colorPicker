...
(function () {
    ...
    var pickerId    = 'color-picker',
        paletteId   = pickerId + '-palette',
        barId       = pickerId + '-bar',
        pindId      = pickerId + '-pind',
        bindId      = pickerId + '-bind',
        cboxId      = pickerId + '-cbox',
        tboxId      = pickerId + '-tbox';
...
proto.show = function() {
	...
	if( typeof(panel) == "undefined" || ! panel ) {
	    ...
	    //panel = this._get_panel();
		this.nodes.panel = panel;
		panel.style.display = "none";
	} else {
		if(panel._resizing == true) { return; }
	}
	var pos = this._get_element_abs_pos(this.nodes.trigger, true);
...
* private methods
* ----------------------------------------------------------------- */
proto._get_panel = function() {
    var panel = document.getElementById(pickerId);
    var p = this.p;
    if (!panel) {
        panel = this._create_panel();
        ...
        this.nodes.panel = panel;
        this.nodes.palette = document.getElementById(paletteId);
        this.nodes.bar = document.getElementById(barId);
        this.nodes.pind = document.getElementById(pindId);
        this.nodes.bind = document.getElementById(bindId);
        this.nodes.cbox = document.getElementById(cboxId);
        this.nodes.tbox = document.getElementById(tboxId);
    }
    return panel;
};
...
proto._create_panel = function() {
	...
	var border_width = 1;
	// outer frame
	var panel = this._create_div_node(pickerId);
	panel._resizing = false;
    panel.className = 'color-picker-panel';
	...
	// color palette
	var palette = this._create_div_node(paletteId);
	...
	// lightness bar
	var bar = this._create_div_node(barId);
	...
	var pind = this._create_div_node(pindId);
	...
	// bar indicator
	var bind = this._create_div_node(bindId);
	...
	if (Ext.is.Desktop) {
	    ...
	    var stopEvent = function(e) {
	        e.preventDefault();
	        e.stopPropagation();
	        console.log("stop event. color picker");
        }
	    _add_event_listener(palette, "touchstart", function(e) { _this._pallet_tapped(e); });
	    _add_event_listener(bar, "touchstart", function(e) { _this._lightness_bar_tapped(e); });
        _add_event_listener(okb, "touchstart", function(e) { stopEvent(e); });
        _add_event_listener(ccb, "touchstart", function(e) { stopEvent(e); });
	    _add_event_listener(okb, "touchend", function(e) { stopEvent(e); _this._ok_clicked(e); });
	    _add_event_listener(ccb, "touchend", function(e) { stopEvent(e); _this._cencel_clicked(e); });
	    _add_event_listener(panel, "touchmove", function(e) {
	        prevent_default(e);
	        stop_propagation(e);
	    });
	}
    // set key event listener
    _add_event_listener(tbox, 'keyup', function(e){
        if (e.keyCode === 13) {
            prevent_default(e);
        	stop_propagation(e);
        	_this.nodes.target.value = _this.nodes.tbox.value;
        	_this.prepare();
        	_this.hide();
        	_this._focus_to_target();
        }
    });
	...
	if (Ext.is.Desktop) {
	    ...
	    _add_event_listener(document, "mousedown", function(e) { _this.hideAll(); });
	    ...
	    // touch
        _add_event_listener(document, "touchstart", function(e) { _this.hideAll(); });
        _add_event_listener(this.nodes.target, "touchstart", function(e) { stop_propagation(e); });
        _add_event_listener(this.nodes.trigger, "touchstart", function(e) { stop_propagation(e); });
        _add_event_listener(panel, "touchstart", function(e) { stop_propagation(e); prevent_default(e);});
        _add_event_listener(panel, "click", function(e) { stop_propagation(e); prevent_default(e);});
        _add_event_listener(okb, "touchstart", function(e) { stop_propagation(e); });
        _add_event_listener(ccb, "touchstart", function(e) { stop_propagation(e); });
	}
	//
	return panel;
};
...
proto._palette_drag_event_prepare = function() {
	...
	if (Ext.is.Desktop) {
	    ...
	    // touch
	    _add_event_listener(palette, "touchstart", function(e) { mouse_down(_this, e); });
	    _add_event_listener(palette, "touchend", function(e) { mouse_up(_this, e); });
	    _add_event_listener(palette, "touchmove", function(e) {
	        var touch = e.changedTouches[0];
	        
	        if(_this.nodes.palette._dragging != true) { return; }
	        prevent_default(e);
	        stop_propagation(e);
	        // clicked coordinates in the palette
	        var pos = _this._get_mouse_position(touch, _this.nodes.palette);
	        
	        _this._pallet_moved(pos.x, pos.y);
	    });
        _add_event_listener(panel, "touchmove", function(e) { mouse_move_panel(_this, e); });
	}
...
proto._lightness_bar_drag_event_prepare = function() {
	...
	if (Ext.is.Desktop) {
	    ...
	    _add_event_listener(bar, "touchstart", function(e) { mouse_down(_this, e); });
	    _add_event_listener(bar, "touchend", function(e) { mouse_up(_this, e); });
	    _add_event_listener(bar, "touchmove", function(e) { 
	        if (_this.nodes.bar._dragging != true) { return; }
	        prevent_default(e);
	        stop_propagation(e);
	        var pos = _this._get_mouse_position(e.changedTouches[0], _this.nodes.bar);
	        _this._lightness_bar_moved(pos.y);
        });
	    //_add_event_listener(panel, "touchmove", function(e) { mouse_move_panel(_this, e); });
	}
};
...
proto._pallet_tapped = function(e) {
    prevent_default(e);
    stop_propagation(e);
    var p = this.p;
    var nodes = this.nodes;
    // clicked coordinates in the palette
    var pos = this._get_mouse_position(e.changedTouches[0], nodes.palette);
    //
    this._pallet_moved(pos.x, pos.y);
};
...
proto._lightness_bar_tapped= function(e) {
    prevent_default(e);
    stop_propagation(e);
    var p = this.p;
    // clicked coordinates in the palette
    var pos = this._get_mouse_position(e.changedTouches[0], this.nodes.bar);
    //
    this._lightness_bar_moved(pos.y);
};
...
proto._create_div_node = function(id) {
...
proto._show_element = function(el, speed) {
	...
	var w = 0;
	var h = 0;
	var scale_up = function scale_up() {
		el._resizing = true;
		w += ( width * speed / 50 );
		h += ( height * speed / 50 );
		if(w > width) { w = width; }
...
proto._get_element_abs_pos = function(el, transformEnable) {
	var o = {};
	...
	while(parent.offsetParent) {
		...
		if (transformEnable) {
		    if (parent.classList.contains('x-scroller')) {
		        scrollEl = parent;
		    }
		}
	}
	// TODO transform対応
	if (scrollEl) {
		var transform = scrollEl.style.getPropertyValue('-webkit-transform');
		if (transform) {
			//console.log('transform found.')
			var matcher = transform.match(/translate3d\((-?\d+)px,\s*(-?\d+)px,\s*-?\d+px\)/);
			if (matcher) {
				//console.log('matcher[1]: ', matcher[1], ', matcher[2]: ', matcher[2]);
				o.left += Number(matcher[1]);
				o.top += Number(matcher[2]);
			}
		}
	}
...
function _prepare_cpick(p, trigger, target) {
	...
	if (Ext.is.Desktop) {
	    _add_event_listener(trigger, "click", listener);
	} else {
	    // touch
        _add_event_listener(trigger, "touchend", listener);
	}
    // set key event listener
    _add_event_listener(target, 'keyup', function(event){
        if (event.keyCode === 13) {
            o.prepare();
            for( var i=0; i<cpicks.length; i++ ) {
                cpicks[i].hide();
...
function _add_event_listener(elm, type, func) {
	var listeners = elm.eventListeners,
		l = {
			el: elm,
			type: type,
			handler: func,
			equals: function(obj){
				return (this.el === obj.el &&
						this.type === obj.type &&
						this.handler.toString() === obj.handler.toString());
			}
	};
